...
export class NewComponent implements OnInit {
  ...
  constructor(
    ...
    this.signUpForm = new FormGroup({
      ...
      enrollIn: new FormControl('Graduate Studies', [Validators.required]),
      program: new FormControl('MAED Major in Administration and Supervision', [
        Validators.required,
      ]),
      semester: this.sem,
    });
  ...
  signUp() {
    ...
    this.collegecontroller.gradaute_sent_otp(this.signUpForm.value).subscribe({
      next: () => {
        this.loadbar = false;
        this.otp = true;
      },
    ...
    this.collegecontroller.verifyotp({ otp: this.enteredOTP || '' }).subscribe({
      next: (response) => {
        ...
        if (this.info[0][0]['expire'] <= myFormattedDate) {
          ...
          this.loading = true;
          this.collegecontroller
            .graduate_studies_create(this.signUpForm.value)
            .subscribe({
              next: (res) => {
                console.log(res);
                this.info = res;
  ...
  ngOnInit(): void {
    ...
    this.semester_controller.graduate_active_sem().subscribe((res) => {
      this.semesterinfo = res;
      if (this.semesterinfo[0]) {
        this.semester = this.semesterinfo[0][0]['semester_name'];
